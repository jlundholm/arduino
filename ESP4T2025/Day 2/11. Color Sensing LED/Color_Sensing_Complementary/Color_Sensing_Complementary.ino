//Author: 
//Program: Color Sensing Complementary
...
void loop() {
  ...
  analogWrite(redpin, gammatable[(int)green]);
  analogWrite(greenpin, gammatable[(int)red]);
  analogWrite(bluepin, gammatable[((int)red+((int)green)/2)/2]);
}