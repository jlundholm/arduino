/* Similar to playSong3 but pace driven by pot and song selection via switch
    Borrowed from:
...
int switchPin = 12;
int potPin = A0;
int nsong = 3; //number of songs in playlist
...
void singsong(int dur[], int mel[]) {
  ...
  for (int thisNote = 0; dur[thisNote] != 0; thisNote++) {
    pace = analogRead(potPin) * 3; //read the pot setting
    if (digitalRead(switchPin) == LOW) {
      song = (song + 1) % nsong;
      delay(1000);  //wait polite second
      return;
    }
